...
class DifferentialDriveRobot:
    ...
    def __init__(self, x=0.0, y=0.0, theta=0.0):
        ...
        self.x = x
        self.y = y
        self.theta = theta  # Góc hướng (rad)
...
def generate_motion_commands(robot, waypoints):
    ...
    Quy trình cho mỗi waypoint:
      1. Tính góc lệch giữa hướng xe và điểm đích
      2. Xoay tại chỗ để căn hướng
    ...
    for i, (tx, ty) in enumerate(waypoints):
        ...
        angle_to_target = compute_angle_to_target(sim_x, sim_y, tx, ty)
        angle_error = normalize_angle(angle_to_target - sim_theta)
        ...
        if abs(angle_error) > 0.01:  # Ngưỡng nhỏ tránh xoay không cần thiết
            ...
            for s in range(steps):
                if angle_error > 0:
                    # Quay trái (counter-clockwise): bánh phải tiến, bánh trái lùi
                    # SL - SR > 0 khi quay CCW => left tiến nhiều hơn? 
                    # delta_theta = (SL - SR) / L > 0 => quay CCW
                    # Xoay tại chỗ: SL = -arc, SR = +arc => delta_theta = (-arc - arc)/L = -2arc/L < 0 SAI
                    # Đúng: muốn delta_theta > 0 (quay CCW) => SL > SR
                    # Xoay tại chỗ: bánh trái tiến, bánh phải lùi? Không...
                    # delta_theta = (SL - SR) / L
                    # Quay CCW (theta tăng): cần SL > SR
                    # Xoay tại chỗ: bánh trái chạy lùi (-), bánh phải chạy tiến (+)
                    # NHƯNG: SL = ticks*d, nếu ticks âm => SL âm
                    # delta_theta = (SL - SR)/L = (-tick*d - tick*d)/L = -2*tick*d/L < 0 => CW
                    # Vậy quay CCW: bánh trái tiến (+), bánh phải lùi (-)
                    # delta_theta = (tick*d - (-tick*d))/L = 2*tick*d/L > 0 => CCW ✓
                    dL = ticks_per_step
                    dR = -ticks_per_step
                else:
...
class AStarPathfinder:
    ...
    def __init__(self, map_width, map_height, grid_size, clearance):
        ...
        self.cols = map_width // grid_size    # Số cột (20)
        self.rows = map_height // grid_size   # Số hàng (20)
    ...
    def grid_to_world(self, col, row):
        ...
        x = col * self.grid_size + self.grid_size / 2.0
        y = row * self.grid_size + self.grid_size / 2.0
        return x, y
    ...
    def build_occupancy_grid(self, obstacles, obstacle_radius, obstacle_rect_size):
        ...
        for (ox, oy, shape, patch_obj, marker) in obstacles:
            if shape == 'circle':
                ...
                min_c = max(0, int((ox - r) // gs))
                max_c = min(self.cols - 1, int((ox + r) // gs))
                min_r = max(0, int((oy - r) // gs))
                max_r = min(self.rows - 1, int((oy + r) // gs))
                ...
                min_c = max(0, int((ox - half) // gs))
                max_c = min(self.cols - 1, int((ox + half) // gs))
                min_r = max(0, int((oy - half) // gs))
                max_r = min(self.rows - 1, int((oy + half) // gs))
                for rr in range(min_r, max_r + 1):
                    for cc in range(min_c, max_c + 1):
                        cx, cy = self.grid_to_world(cc, rr)
...
class OdometrySimulator:
    ...
    def _update_info(self):
        ...
        info = (
            ...
            f" X      : {x:>9.2f} mm\n"
            f" Y      : {y:>9.2f} mm\n"
            f" θ      : {math.degrees(theta):>9.2f} °\n"
            f" θ (rad): {theta:>9.4f} rad\n"
            f"------------------------------\n"
            f" ENCODER \n"
    ...
    def _on_astar_click(self, tx, ty):
        ...
        for i, (wx, wy) in enumerate(astar_waypoints):
            state = SimState(self.planning_x, self.planning_y, self.planning_theta)
